...
def dataset_split(dataframe,y,scale):
	if scale==True:
		dataframe=robust_scalint(dataframe)
	#else:
	#	print(dataframe)
...
if __name__=='__main__':
	...
	#dataframe,y=get_feature(user+path+file_csv,feature,y_label)
	#plot feature
	#feature_plot(feature,dataframe,y)
	
	#X_train , X_test , Y_train , Y_test = dataset_split(dataframe,y,False)
	...
	#x_mean , x_mode, y = get_main_features(user+path+file_csv , main_features, y_label);
	...
	#esempio uso select K best
	#for i in range(1,len(feature)+1):
	i=8
	#	print("---------------------------------------------------------")
	#	print("\nITERAZIONE: ",i)
	x_mean, x_mode, y = get_main_features(user+path+file_csv , feature , y_label, i)
