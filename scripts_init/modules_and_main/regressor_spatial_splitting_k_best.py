...
def save_tuning_par(filename,knn,dt,rf,i):
...
for i in routes:
	...
	print(filename)
	fullname = os.path.join('../QoS_RAILWAY_PATHS_REGRESSION/', filename)
	x_mean , x_mode , y , main_feature_mean , main_feature_mode=pr.get_main_features(fullname, feature , y_label, 3)
	...
	if x_mean.shape[0]>100:
		...
		print(dt_dict)
		print(rf_dict)
		file_to_save_name = "tuning_parameter_route"
		save_tuning_par(file_to_save_name,knn_dict,dt_dict,rf_dict,i)
		print('-------------------- Mean---------------------------------')
